!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Program:                                                                          !
!   Using Morokuma's IRC algorithm to generate IRC path, and then compare with the  !
!   Gaussian result                                                                 !                                                                           !
!                                                                                   !
! Reference:                                                                        !
!   1. The Journal of Chemical Physics 1977, 66, 2153                               !
!       https://f1000.com/work/item/5428978/resources/4431782/pdf                   !
!   2. GitHub with python code                                                      !
!       https://github.com/teixeirafilipe/irc4orca/blob/master/irc4orca.py          !
!                                                                                   !  
! Input:                                                                            !
!   $1 = Q0.out (gaussian output file w/ force and freq=hpmodes)                    !
!   $2 = Q1.out (gaussian output file w/ force and freq=hpmodes)                    !
!   $3 = 0 or 1 (0 = TS, 1 = other point)                                           !
!   $4 = *.com (gaussian input file; assign output name)                            !
!                                                                                   !
! Output:                                                                           !
...
! History:                                                                          !
!   2018/08/16, Grace                                                               !
!   2018/08/21, Grace, follow paper's algorithm                                     !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
implicit none
    ...
    ! Step 1. Using variable 'NAtoms' to allocate memory
        call GETARG(1,filename)
            ...
            allocate(G0(NAtoms,3))
            allocate(G1(NAtoms,3))
            allocate(D(NAtoms,3))
            allocate(eigenVec(NAtoms,3))
    ! Step 2. Extract gradient of point Q0 and Q1
        ! Point Q0
        ...
        call GETARG(1,filename)
            if ( i .eq. 0 ) then 
                ...
                call getG(filename,NAtoms,G0)
            end if 
            call normVec(NAtoms,3,G0)
        ! Point Q1
        call GETARG(2,filename)
            call getG(filename,NAtoms,G1)
            call normVec(NAtoms,3,G1)
    ! Step 3. Formulate vector D
            call getD(NAtoms,3,G0,G1,D)
            call normVec(NAtoms,3,D)
    ! Step 4. Generate the next structure
            call getCoord(filename,NAtoms,AtomName,iniCoord)
            ! call changeCar2MW(NAtoms,3,AtomName,iniCoord)
            call genGeom(NAtoms,ds,D,iniCoord,finCoord)
...
subroutine getD(dimen1,dimen2,G0,G1,D)
    implicit none
    integer(4),intent(in)   :: dimen1,dimen2
    real(8),dimension(dimen1,dimen2),intent(in) :: G0,G1
    real(8),dimension(dimen1,dimen2),intent(inout) :: D
    ! local variable
    integer(4)  :: i,j
    D = 0.0D0
    do i = 1,dimen1
